...
impl Store {
    ...
    /// Ok(true) if this was genuinely new, Ok(false) if paging_token was
    /// already seen — the UNIQUE constraint on paging_token *is* the
    /// idempotency guarantee, not application-level dedup logic.
    ///
